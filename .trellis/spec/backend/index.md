...
4. Read [HTTP Contracts](./http-contracts.md) before changing collection pagination or product mutations.
5. Read [Logging Guidelines](./logging-guidelines.md) before changing startup behavior or auditable mutations.
6. Preserve the API contract used by `apps/web/src/features/types.ts` and `apps/web/src/lib/api.ts`.
