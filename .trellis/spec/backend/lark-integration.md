...
- Presentation is a validated whitelist (`auto`, summary, detail, ranking, matrix, trend, share, comparison). The server selects titles, colors, fields, images, values, bars, percentages, empty/truncated text, filters, and time context from real query results.
- DeepSeek parsing has a 10-second total deadline. Transport/timeouts, 429, and 5xx retry once; other HTTP failures, malformed responses, malformed plan JSON, and invalid/unsupported plans do not retry. Structured logs contain only message ID, stage, status, attempts, and elapsed time.
...
| Model response/plan is malformed, unsupported, or requests more than two dimensions | Return a distinct not-understood/unsupported card; do not execute a query or stock operation |
...
| Missing/unknown domain, metric, operation, group, date, comparison, presentation, incompatible combination, limit outside 1–10, or an overlong filter | Reject the complete analytics plan; do not fall back to a similar query |
...
- Retry tests cover transport timeout, 429, 5xx, non-retryable 4xx, malformed response/plan, caller deadline, attempts/elapsed metadata, failure-stage classification, and secret/input/response redaction.
