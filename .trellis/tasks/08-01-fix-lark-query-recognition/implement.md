...
- [x] 使用 10 秒总 context 和最多两次请求；网络/超时/429/5xx 可重试，`plan_validate` 可在同一预算内纠正一次。
- [x] 在 handle 层带 `message_id` 记录最终失败或重试恢复，确保不记录原文/响应/密钥。
- [x] 区分上游暂时异常卡片与计划未理解/不支持卡片。
- [x] 在总请求上限不变的前提下允许一次 `plan_validate` 自纠正，并记录恢复阶段。
...
- [x] 放开安全字段内的 operation/presentation 组合标签，按 metric/group_by 归一且不丢查询条件。
...
- [x] 回放“今天出库了哪些商品，多少数量”及近似表达，确认生成今日商品出库数量排行。
