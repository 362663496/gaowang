...
模型不执行 SQL、不读取数据库、不生成最终数字，也不直接生成飞书卡片 JSON。一次模型调用同时决定查询语义和白名单展示意图；查询后不追加第二次模型调用。
...
- 展示类型与操作不匹配时返回计划不支持，不静默丢弃维度；`auto` 才由服务端选择对应模板。
...
- 仅 transport/timeout、429、5xx 重试一次，短暂等待使用标准库 timer 并响应 context 取消。
- 其他 4xx、响应 JSON、finish reason、模型计划 JSON 和白名单校验错误只请求一次。
...
- 500/429/超时后成功，以及 4xx/坏 JSON/坏计划不重试和 10 秒总截止。
