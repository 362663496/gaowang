...
- 收敛命令解析：保留 `查库存`，为 `查商品` 返回迁移提示，增加 `库存概览`、可选参数 `查流水` 和 `今日变动`。
- 仅在本地命令未匹配时调用 DeepSeek，并在最终意图确定后写审计。
...
- 更新帮助、迁移、AI 降级、概览、今日变动和流水卡片测试；断言商品图位于 `extra`，避免回归为全宽图片。
...
- 部署前只将 DeepSeek Key 写入服务器共享环境文件；重启 API 后验证健康、长连接、固定命令、自然语言、图片、错误降级和审计。
